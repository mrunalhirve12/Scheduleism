...
class Node:
    def __init__(self, time, color, parent, left=None, right=None):
        self.time = time
        self.queue = deque([]) #Added queue to hold multiple processes with same time
        self.color = color
        self.parent = parent
        self.left = left
        self.right = right
    ...
    def __repr__(self):
        return '{color} {val} Node'.format(color=self.color, val=self.time)
    ...
    def __eq__(self, other):
        ...
        if self.parent is None or other.parent is None:
            ...
            parents_are_same = self.parent.time == other.parent.time and self.parent.color == other.parent.color
        return self.time == other.time and self.color == other.color and parents_are_same
...
class RedBlackTree:
    ...
    NIL_LEAF = Node(time=None, color=NIL, parent=None)
    ...
    def add(self, time):
        if not self.root:
            self.root = Node(time, color=BLACK, parent=None, left=self.NIL_LEAF, right=self.NIL_LEAF)
            self.count += 1
            return
        parent, node_dir = self._find_parent(time)
        if node_dir is None:
            return  # time is in the tree
        new_node = Node(time=time, color=RED, parent=parent, left=self.NIL_LEAF, right=self.NIL_LEAF)
        if node_dir == 'L':
    ...
    def remove(self, time):
        ...
        """
        node_to_remove = self.find_node(time)
        if node_to_remove is None:  # node is not in the tree
            return
        
        if len(node_to_remove.queue) == 0: #Make sure there's no process in the node queue before removing
            if node_to_remove.get_children_count() == 2:
                # find the in-order successor and replace its time.
    ...
    def contains(self, time) -> bool:
        """ Returns a boolean indicating if the given time is present in the tree """
        return bool(self.find_node(time))

    def ceil(self, time) -> int or None:
        """
        Given a time, return the closest time that is equal or bigger than it,
        ...
        def find_ceil(node):
            ...
            if node.time == time:
                ...
                # this node is bigger, save its time and go left
                last_found_val = node.time
    ...
    def _remove(self, node):
        ...
        if node == self.root:
            ...
            if not_nil_child.color == RED:
                """
                Swap the times with the red child and remove it  (basically un-link it)
    ...
    def _try_rebalance(self, node):
        ...
        node_dir = 'L' if parent.time > time else 'R'
        parent_dir = 'L' if grandfather.time > parent.time else 'R'
    ...
    def __update_parent(self, node, parent_old_child, new_parent):
        ...
        if new_parent:
            ...
            if new_parent.time > parent_old_child.time:
    ...
    def _right_rotation(self, node, parent, grandfather, to_recolor=False):
        ...
        grandfather.left = old_right  # save the old right times
    ...
    def _left_rotation(self, node, parent, grandfather, to_recolor=False):
        ...
        grandfather.right = old_left  # save the old left times
    ...
    def _find_parent(self, time):
        """ Finds a place for the time in our binary tree"""
    ...
    def display(self):
        def inner_find(root):
            if root is None or root == self.NIL_LEAF:
                ...
                inner_find(root.left)
                print(root.printQueue())
